  ...
  divisionChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Vehicles',
        data: data,
        backgroundColor: ['#0f766e','#14b8a6','#06b6d4','#3b82f6','#8b5cf6','#ec4899'],
...
function updateCarInspectionStatus() {
  ...
  const sd = analyticsData.inspectionStatus || {};
  const labels = Object.keys(sd);
  const data = Object.values(sd);
  const total = data.reduce((a, b) => a + b, 0);

  if (!labels.length || total === 0) { ctx.getContext('2d').clearRect(0, 0, ctx.width, ctx.height); return; }

  inspectionStatusChart = new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: labels,
      datasets: [{ data: data, backgroundColor: ['#f59e0b','#94a3b8','#10b981'], borderWidth: 0 }]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: {
...
function updateDashboardVehicleTable() {
  ...
  tbody.innerHTML = vehiclesData.map(function(v) {
    var last = inspectionsData.filter(function(i){ return i.vehicleId === v.id; }).sort(function(a,b){ return new Date(b.inspectionDate) - new Date(a.inspectionDate); })[0];
    var badge;
    if (last) {
      badge = last.overallStatus === 'Passed'
        ? '<span style="background:#10b981;color:white;padding:4px 12px;border-radius:12px;font-size:12px;"> Passed</span>'
...
function updateCarHistory() {
  ...
  tbody.innerHTML = sorted.map(function(i) {
    var badge = i.overallStatus==='Passed'
      ? '<span style="background:#10b981;color:white;padding:6px 14px;border-radius:12px;font-size:12px;font-weight:600;">✓ Passed</span>'
...
function handleVehicleSelection() {
  ...
  sel.addEventListener('change', function() {
    ...
    var v = vehiclesData.find(function(x){ return x.id === vid; });
    if (!v) return;
...
function viewInspectionDetails(inspectionId) {
  ...
  equipmentFields.forEach(function(field) {
    ...
    if (condition !== 'N/A' && condition !== '') {
      var statusBadge = condition === 'OK' 
        ? '<span style="background:#10b981;color:white;padding:3px 10px;border-radius:6px;font-size:11px;">✓ OK</span>'
  ...
  var overallBadge = inspection.overallStatus === 'Passed'
    ? '<span style="background:#10b981;color:white;padding:8px 20px;border-radius:8px;font-size:14px;font-weight:600;">✓ Passed</span>'
  ...
  var modalContent = '<div style="max-width:' + (isMobile ? '100%' : '900px') + ';margin:0 auto;background:white;border-radius:' + (isMobile ? '0' : '12px') + ';overflow:hidden;max-height:' + (isMobile ? '100vh' : '90vh') + ';display:flex;flex-direction:column;">' +
    '<div style="background:linear-gradient(135deg, #0f766e 0%, #14b8a6 100%);color:white;padding:' + (isMobile ? '16px' : '24px 30px') + ';display:flex;justify-content:space-between;align-items:center;">' +
      '<div>' +
